...
import (
	...
	userusecase "github.com/mmtaee/ocserv-dashboard/backend/internal/usecase/user"
...
type Controller struct {
	...
	systemRepo      *systemusecase.Usecase
	userRepo        *userusecase.Usecase
	captchaVerifier captcha.GoogleCaptchaInterface
	cryptoRepo      crypto.CustomPasswordInterface
}

func New(systems *systemusecase.Usecase, users *userusecase.Usecase, captchaVerifier captcha.GoogleCaptchaInterface, passwordCrypto crypto.CustomPasswordInterface) *Controller {
	return &Controller{
		request:         request.NewCustomRequest(),
		systemRepo:      systems,
		userRepo:        users,
		captchaVerifier: captchaVerifier,
		cryptoRepo:      passwordCrypto,
	}
}
...
func (ctl *Controller) Login(c *echo.Context) error {
	...
	now := time.Now()
	user.LastLogin = &now
	_ = ctl.userRepo.UpdateLastLogin(c.Request().Context(), user)
