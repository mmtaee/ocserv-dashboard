...
import (
	...
	tg18n "github.com/mmtaee/ocserv-dashboard/backend/services/admin_api/internal/telegram/i18n"
)

type Controller struct {
	request        request.CustomRequestInterface
	repo           *telegramusecase.Usecase
	ocservUserRepo *userusecase.Usecase
}
...
func New(telegramUsecase *telegramusecase.Usecase, userUsecase *userusecase.Usecase) *Controller {
	tg18n.Init()
	...
	return &Controller{
		...
		repo:           telegramUsecase,
		ocservUserRepo: userUsecase,
	}
}
...
func (ctl *Controller) Approve(c *echo.Context) error {
	...
	ctl.notifyAwaitingPayment(c.Request().Context(), updated, &awaitingPaymentOpts{
...
func (ctl *Controller) Reject(c *echo.Context) error {
	...
	ctl.notifyRejected(c.Request().Context(), updated)
	return c.JSON(http.StatusOK, updated)
}
...
func (ctl *Controller) deliverNewAccount(
	...
	if err := ctl.linkTelegramAccount(c.Request().Context(), req.ChatID, req.TelegramUsername, settings.DefaultLanguage, created.ID); err != nil {
	...
	ctl.notifyDelivery(req.ChatID, settings, formatNewAccountMessage(settings, created, password, expireAt))
...
func (ctl *Controller) deliverRenewal(
	...
	ctl.notifyDelivery(req.ChatID, settings, formatRenewalMessage(settings, user, newExpire))
	return c.JSON(http.StatusOK, map[string]interface{}{
		"status":   "delivered",
...
func (ctl *Controller) findOcservUserByID(ctx context.Context, id uint) (*models.OcservUser, error) {
	return ctl.repo.OcservUserByID(ctx, id)
}
...
func (ctl *Controller) notifyAwaitingPayment(ctx context.Context, req *models.TelegramRequest, opts *awaitingPaymentOpts) {
	settings, err := ctl.repo.Settings(ctx)
	if err != nil || settings.BotToken == "" || !settings.Enabled {
		return
	}
	lang := ctl.resolveNotifyLang(ctx, req.ChatID, settings)
	var pkg *models.TelegramPackage
	if req.PackageID != nil && *req.PackageID > 0 {
		if p, err := ctl.repo.PackageByID(ctx, *req.PackageID); err == nil {
			pkg = p
		}
	}
	...
	_ = ctl.repo.SetAwaitingPaymentMessageID(ctx, req.ID, msgID)
}
...
func (ctl *Controller) notifyRejected(ctx context.Context, req *models.TelegramRequest) {
	settings, err := ctl.repo.Settings(ctx)
	if err != nil || settings.BotToken == "" || !settings.Enabled {
		return
	}
	if req.AwaitingPaymentMessageID != nil && *req.AwaitingPaymentMessageID > 0 {
		...
		_ = ctl.repo.ClearAwaitingPaymentMessageID(ctx, req.ID)
	}
...
func (ctl *Controller) linkTelegramAccount(ctx context.Context, chatID int64, username, language string, ocservUserID uint) error {
	if language == "" {
	...
	return ctl.repo.LinkAccount(ctx, account)
}
