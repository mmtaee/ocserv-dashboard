		...
		return tx.Exec(`
			CREATE TABLE telegram_settings (
				...
				default_language VARCHAR(8) NOT NULL DEFAULT 'en'
					CHECK (default_language IN ('en', 'fa', 'ar', 'ru', 'zh-cn', 'zh-tw', 'it')),
			...
			CREATE TABLE telegram_accounts (
				...
				language VARCHAR(8) NOT NULL DEFAULT 'en'
					CHECK (language IN ('en', 'fa', 'ar', 'ru', 'zh-cn', 'zh-tw', 'it')),
			...
			CREATE TABLE telegram_requests (
				...
				chat_id BIGINT NOT NULL,
				telegram_username VARCHAR(64) NOT NULL DEFAULT '',
				type VARCHAR(16) NOT NULL CHECK (type IN ('new', 'renew')),
				package_id BIGINT NULL,
				target_ocserv_user_id BIGINT NULL,
				desired_username VARCHAR(64) NOT NULL DEFAULT '',
				status VARCHAR(32) NOT NULL DEFAULT 'pending'
					CHECK (status IN ('pending', 'awaiting_payment', 'payment_uploaded', 'approved', 'rejected', 'delivered')),
