...
func (s *StatService) CalculateUserStats() error {
	for {
		select {
		case <-s.ctx.Done():
...
func (s *StatService) saveRxTx(ctx context.Context, u *UserStats) error {
	...
	ocUser, err := s.repository.FindUser(ctx, u.Username)
	if err != nil {
	...
	totalMonthRX, totalMonthTX, err := s.repository.CurrentMonthTotals(ctx, ocUser.ID, ocUser.UsageResetAt)
	if err != nil {
	...
	case models.MonthlyReceive:
		shouldLock = int64(totalMonthRX) >= trafficSizeBytes

	case models.MonthlyRxTx:
		shouldLock = int64(totalMonthRX)+int64(totalMonthTX) >= trafficSizeBytes
	...
	if shouldLock && !wasLocked {
		err = s.access.Disconnect(ocUser.Username)
		if err != nil {
			logger.Error("Error disconnecting user: %v", err)
		}

		err = s.access.Lock(ocUser.Username)
		if err != nil {
...
func (s *StatService) saveSessionLog(ctx context.Context, log *models.OcservUserSessionLog) error {
	err := s.repository.SaveSessionLog(ctx, log)
	if err != nil {
		logger.Error("Error updating user stats: %v", err)
		return err
	}
