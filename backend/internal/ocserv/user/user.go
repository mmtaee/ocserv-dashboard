...
import (
	...
	"github.com/mmtaee/ocserv-dashboard/backend/internal/ocserv/configfile"
	"os"
	"os/exec"
	"strings"
)
...
func (u *OcservUser) Create(group, username, password string, config *models.OcservUserConfig) error {
	args := []string{"-c", configfile.OcpasswdPath, username}
	if group != "" && group != "defaults" {
		args = append([]string{"-g", group}, args...)
	}
	cmd := exec.Command(configfile.OcpasswdExec, args...)
...
func (u *OcservUser) Lock(username string) (string, error) {
	output, err := configfile.RunOcpasswd("-l", "-c", configfile.OcpasswdPath, username)
	if err != nil {
		return "", err
	}
...
func (u *OcservUser) UnLock(username string) (string, error) {
	output, err := configfile.RunOcpasswd("-u", "-c", configfile.OcpasswdPath, username)
	if err != nil {
		return "", err
	}
...
func (u *OcservUser) Delete(username string) (string, error) {
	...
	output, err := configfile.RunOcpasswd("-d", "-c", configfile.OcpasswdPath, username)
	if err != nil {
		return "", err
	}
...
func (u *OcservUser) SyncConfig(username, group string, config *models.OcservUserConfig) error {
	filename := configfile.UserConfigFilePathCreator(username)

	if err := os.MkdirAll(configfile.ConfigUserBaseDir, 0750); err != nil {
		return err
	}
	...
	groupConfig := configfile.GroupConfigFilePathCreator(group)
...
func hasConfigValues(config *models.OcservUserConfig) bool {
	...
	for _, value := range configfile.ToMap(config) {
		switch v := value.(type) {
		case nil:
			continue
		case bool:
...
func (u *OcservUser) CreateConfig(username string, config *models.OcservUserConfig) error {
	...
	return configfile.ConfigWriter(file, configfile.ToMap(config))
}
...
func (u *OcservUser) Ocpasswd(ctx context.Context) (*[]Ocpasswd, int, error) {
	f, err := os.Open(configfile.OcpasswdPath)
	if err != nil {
		return nil, 0, err
	}
	defer f.Close()
	...
	total, err := OcpasswdTotalLines(configfile.OcpasswdPath)
	if err != nil {
		total = 0
	}
