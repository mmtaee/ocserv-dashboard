...
func (s *Service) registerSystemRoutes(e *echo.Group) {
	...
	admin.POST("/users/:uid/password", s.system.ChangeUserPasswordByAdmin)
	admin.DELETE("/users/:uid", s.system.DeleteUser)
}
...
func (s *Service) registerUserRoutes(e *echo.Group) {
	...
	g.GET("", s.users.Users)
	g.GET("/:uid", s.users.User)
	g.POST("", s.users.Create)
	g.PATCH("/:uid", s.users.Update)
	g.DELETE("/:uid", s.users.Delete)
	...
	g.POST("/:uid/lock", s.users.Lock)
	g.POST("/:uid/unlock", s.users.UnLock)
	g.POST("/:uid/activate", s.users.ActivateExpired)
	g.POST("/:uid/certificate", s.users.CreateCertificate)
	g.GET("/:uid/certificate", s.users.DownloadCertificate)
	g.GET("/:uid/session_logs", s.users.SessionLogs)
	g.GET("/:uid/statistics", s.users.Statistics)
