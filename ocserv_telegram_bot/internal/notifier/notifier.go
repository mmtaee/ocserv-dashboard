...
func (n *Notifier) scan(ctx context.Context) {
	...
	if err != nil {
		logger.Warn("ocserv_telegram_bot: notifier list accounts: %v", err)
		return
	}
	...
	for _, account := range accounts {
		...
		if err := n.sender.Send(account.ChatID, text); err != nil {
			logger.Warn("ocserv_telegram_bot: notifier send failed: %v", err)
			continue
		}
		if err := n.repo.MarkLowQuotaNotified(ctx, account.ID, now); err != nil {
			logger.Warn("ocserv_telegram_bot: notifier mark notified: %v", err)
		}
	}
}
