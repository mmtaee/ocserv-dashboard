...
func (m *Manager) refresh(ctx context.Context) {
	...
	if err != nil {
		logger.Warn("ocserv_telegram_bot: failed to load settings: %v", err)
		return
	}
	...
	if !desiredEnabled || desiredToken == "" {
		logger.Info("ocserv_telegram_bot: idle (enabled=%v token_set=%v)", desiredEnabled, desiredToken != "")
		return
	}

	api, err := tgbotapi.NewBotAPI(desiredToken)
	if err != nil {
		logger.Error("ocserv_telegram_bot: failed to init bot api: %v", err)
		return
	}
	...
	logger.Info("ocserv_telegram_bot: connected as @%s", api.Self.UserName)
}
	...
	for {
		...
		case upd, ok := <-updates:
			...
			func() {
				defer func() {
					if r := recover(); r != nil {
						logger.Error("ocserv_telegram_bot panic: %v", r)
					}
				}()
