...
# Parameters:
#   $1 - minimum Go version (default: 1.23)
# ===============================
check_go_version() {
    local go_mod_file="../../core/go.mod"

    if [[ ! -f "$go_mod_file" ]]; then
        die "❌ go.mod not found at $go_mod_file"
    fi

    # Extract Go version from the go.mod file (e.g., "1.23")
    ...
    # Get current Go version (e.g., 1.23.5)
...
# Description:
#   Ensures Node.js v20.x or higher exists.
