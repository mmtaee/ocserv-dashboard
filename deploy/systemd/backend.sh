...
# Script: backend.sh
# Description:
#   Main deployment script for ocserv_dashboard backend services.
#
...
#   - Builds Go services (admin-api, customer-api, ocserv_log_parser, ocserv_user_manager, ocserv_telegram_bot)
...
# -----------------------
declare -A SERVICES=(
  ["admin-api"]="../../admin_dashboard/api"
  ["customer-api"]="../../customer_dashboard/api"
  ["ocserv-log-parser"]="../../ocserv_log_parser"
  ["ocserv-user-manager"]="../../ocserv_user_manager"
  ["ocserv-telegram-bot"]="../../ocserv_telegram_bot"
)
...
for service in "${!SERVICES[@]}"; do
  ...
  (
    ...
    CGO_ENABLED=0 GOOS=linux GOARCH="${ARCH}" go build -ldflags="-s -w" -o "$service"
...
for service in "${!SERVICES[@]}"; do
  ...
  case "$service" in
    admin-api)         ARGS="serve --host 127.0.0.1 --port 8080" ;;
    customer-api)      ARGS="serve --host 127.0.0.1 --port 8081" ;;
    ocserv-log-parser) ARGS="" ;;
    ocserv-user-manager) ARGS="" ;;
    ocserv-telegram-bot) ARGS="" ;;
    *)                 ARGS="" ;;
  esac
