...
async function submit(credentials: GithubComMmtaeeOcservDashboardBackendInternalServicesAdminApiSystemLoginData): Promise<void> {
  if (await auth.signIn(credentials)) {
    await router.replace({ name: systemInit.isInitialized ? 'home' : 'system-setup' })
  }
}
</script>
  ...
  <div class="grid min-h-svh lg:grid-cols-[minmax(0,1.25fr)_minmax(22rem,.75fr)]">
    <div class="flex flex-col gap-6 p-6 md:p-10">
      <div class="flex items-center gap-2 font-medium">
        <span class="grid size-7 place-items-center rounded-md bg-primary text-xs text-primary-foreground" aria-hidden="true">OC</span>
        {{ t('common.appName') }}
    ...
    <aside class="relative hidden overflow-hidden bg-zinc-950 text-white lg:flex lg:items-center lg:justify-center lg:p-12">
      ...
      <div class="relative flex max-w-md flex-col items-center gap-6 text-center">
        ...
        <div class="flex flex-col gap-3">
          ...
          <p class="text-sm leading-6 text-white/65">{{ t('auth.promoDescription') }}</p>
        </div>
      </div>
    </aside>
  </div>
