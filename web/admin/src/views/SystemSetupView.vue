...
import {type SystemUpdateData, updateSystemConfig} from '@/api/services/system'
  ...
  <div class="grid min-h-svh lg:grid-cols-[minmax(0,1.25fr)_minmax(22rem,.75fr)]">
    <div class="flex flex-col gap-6 p-6 md:p-10">
      <div class="flex items-center gap-2 font-medium">
        <span class="grid size-7 place-items-center rounded-md bg-primary text-xs text-primary-foreground"
              aria-hidden="true">OC</span>
        {{ t('common.appName') }}
    ...
    <aside
      ...
      <div class="relative flex max-w-md flex-col items-center gap-6 text-center">
        ...
        <div class="flex flex-col gap-3">
          ...
          <p class="text-sm leading-6 text-white/65">{{ t('setup.promoDescription') }}</p>
        </div>
      </div>
    </aside>
  </div>
