import { clearAccessToken, requireAuthorizationHeader, setAccessToken } from '@/api/auth-token'
...
export async function logout(): Promise<void> {
  try {
    await api.auth.authLogoutPost({ authorization: requireAuthorizationHeader() })
  } finally {
    clearAccessToken()
  }
}
