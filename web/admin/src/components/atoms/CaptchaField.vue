...
function loadRecaptcha(): Promise<RecaptchaApi> {
  ...
  return new Promise((resolve, reject) => {
    const existing = document.querySelector<HTMLScriptElement>('script[data-ocserv-recaptcha]')
    const script = existing ?? document.createElement('script')
    const resolveApi = () => window.grecaptcha
      ? resolve(window.grecaptcha)
      : reject(new Error('Captcha failed to load.'))

    script.addEventListener('load', resolveApi, { once: true })
    script.addEventListener('error', () => reject(new Error('Captcha failed to load.')), { once: true })
...
watch(() => props.siteKey, () => void renderCaptcha())
